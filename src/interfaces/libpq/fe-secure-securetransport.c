					  ...
					  CFArrayRef *key_array, CFArrayRef *rootcert_array)
{
	...
	SecIdentityRef		root_identity = NULL;
	SecCertificateRef	root_ref;
		...
		pg_strncasecmp(conn->sslrootcert, KC_PREFIX, KC_PREFIX_LEN) == 0)
	{
		...
		if (keychains)
			import_certificate_keychain(fnbuf, &root_identity,
										(SecKeychainRef) CFArrayGetValueAtIndex(keychains, 0));
		if (!root_identity)
			import_certificate_keychain(fnbuf, &root_identity, NULL);

		if (root_identity)
		{
			SecIdentityCopyCertificate(root_identity, &root_ref);
			if (root_ref)
				conn->st_rootcert = (void *) CFArrayCreate(NULL, (const void **) root_ref, 1, &kCFTypeArrayCallBacks);
		}
	}
		...
		pg_strncasecmp(conn->sslcert, KC_PREFIX, KC_PREFIX_LEN) == 0)
	{
		...
		if (keychains)
			import_certificate_keychain(fnbuf, &identity,
										(SecKeychainRef) CFArrayGetValueAtIndex(keychains, 0));
		if (!identity)
			import_certificate_keychain(fnbuf, &identity, NULL);
