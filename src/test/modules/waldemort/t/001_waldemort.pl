...
sub generate
{
	...
	command_ok([ 'waldemort', '--output', "$dir/$walfile", @options ],
		"generate $name");
	return "$dir/$walfile";
}
...
my @synthetic = (
	...
	'--records', '6',
	'--seed', '17',
	'--types', 'noop,restore-point',
	'--payload-size', $blocksize + 17,);
...
my $empty = generate('empty', '--seg-size', '1', '--records', '0');
command_like([ 'pg_waldump', '--limit', '1', $empty ],
	qr/SWITCH/, 'zero synthetic records produces a switch-only segment');

my $high_lsn = "$tempdir/000000030000000100000001";
command_ok(
	[
		'waldemort', '--output', $high_lsn, '--seg-size',
		'1', '--segno', '4097', '--timeline',
		'3', '--system-id', '42', '--prev',
		'1/0', '--records', '0',
	],
	'generate with explicit identity and an LSN above 4 GB');
command_like(
	[ 'pg_waldump', '--limit', '1', $high_lsn ],
	qr/lsn: 1\/0010[0-9A-F]{4}, prev 1\/00000000/,
	'segment numbering and explicit previous LSN are honored');

my $mixed = generate('mixed', @synthetic, '--mode', 'mixed');
is(-s $mixed, $segsize, 'mixed segment has the requested size');
command_like([ 'pg_waldump', '--limit', '1', $mixed ],
	qr/NOOP/, 'mixed segment begins with a valid record');
command_fails_like(
	[ 'pg_waldump', $mixed ],
...
my ($dump, $stderr);
IPC::Run::run(
	[ 'pg_waldump', '--limit', '1', $valid ],
	'>' => \$dump,
	'2>' => \$stderr) or die "pg_waldump: $stderr";
...
my $garbage =
  generate('garbage', '--mode', 'garbage', '--seg-size', '1', '--seed', '42');
is(-s $garbage, $segsize, 'garbage segment has the requested size');
command_fails_like(
	[ 'pg_waldump', $garbage ],
	qr/invalid magic number|invalid WAL segment size|could not find a valid record/,
	'garbage is not readable WAL');
my $garbage_again = generate(
	'garbage_again', '--mode', 'garbage', '--seg-size',
	'1', '--seed', '42');
is(compare($garbage, $garbage_again), 0,
	'garbage generation is reproducible');
my $different = generate('different', '--mode', 'garbage',
	'--seg-size', '1', '--seed', '43');
isnt(compare($garbage, $different), 0, 'different seeds change garbage');

my $flipped = generate(
	'flipped', '--mode', 'bitflip', '--input', $valid, '--offset',
	'100', '--mask', '128');
my $expected = $original;
substr($expected, 100, 1) = chr(ord(substr($expected, 100, 1)) ^ 128);
ok( read_binary($flipped) eq $expected,
	'bitflip changes exactly the requested bit');
my $unflipped = generate(
	'unflipped', '--mode', 'bitflip', '--input',
	$flipped, '--offset', '100', '--mask',
	'128');
is(compare($valid, $unflipped), 0, 'repeating bitflip restores the original');

my $bad_magic = generate('bad_magic', '--mode', 'bitflip',
	'--input', $valid, '--offset', '0');
my $fixed_magic = generate('fixed_magic', '--mode', 'bitflip',
	'--input', $bad_magic, '--offset', '0');
is(compare($valid, $fixed_magic),
	0, 'raw mutation also accepts a segment with a damaged header');
...
my @bad_options = (
	...
	[ '--seg-size', '3' ],
	[ '--records', '-1' ],
	[ '--seg-size', '1', '--records', '1000', '--payload-size', $blocksize ],
	...
	[
		'--mode', 'bitflip', '--input', $valid, '--offset', '0', '--mask',
		'0'
	],
...
command_fails(
	[
		'waldemort', '--mode', 'bitflip', '--input',
		$valid, '--output', $valid, '--offset',
		'0',
	],
	'refuse to overwrite input');
command_fails(
	[ 'waldemort', '--output', $valid ],
